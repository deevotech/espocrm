...
if (isset($_POST['submit'])) {
	if ($_POST['A'] == 0) {
		...
		include '\PhpOffice\PhpSpreadsheet\Spreadsheet/Calculation.php';
		...
		$discriminant = \PhpOffice\PhpSpreadsheet\Calculation\Calculation::getInstance()->calculateFormula($discriminantFormula);

		$r1Formula = '=IMDIV(IMSUM(-'.$_POST['B'].',IMSQRT('.$discriminant.')),2 * '.$_POST['A'].')';
		$r2Formula = '=IF('.$discriminant.'=0,"Only one root",IMDIV(IMSUB(-'.$_POST['B'].',IMSQRT('.$discriminant.')),2 * '.$_POST['A'].'))';

		echo \PhpOffice\PhpSpreadsheet\Calculation\Calculation::getInstance()->calculateFormula($r1Formula).'<br />';
		echo \PhpOffice\PhpSpreadsheet\Calculation\Calculation::getInstance()->calculateFormula($r2Formula).'<br />';
